...
def test_test_dockerignore_is_sensitive_allowlist_for_copy_sources():
    ...
    assert [line for line in ignore_lines if "docs/" in line] == [
        "!docs/", "docs/*", "!docs/production-web-release.md",
    ]
    assert (
        "COPY docs/production-web-release.md docs/production-web-release.md"
        in dockerfile
    )
    assert {
        ".env*",
