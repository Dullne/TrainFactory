"""Add an immutable execution-attempt fence to evaluation tasks.

Revision ID: 050_add_evaluation_run_token
Revises: 049_dataset_lineage_edge_key
"""
...
revision = "050_add_evaluation_run_token"
down_revision = "049_dataset_lineage_edge_key"
branch_labels = None
depends_on = None

TABLE_NAME = "evaluation_tasks"
COLUMN_NAME = "run_token"
INDEX_NAME = "idx_eval_run_token"
...
def _schema_snapshot(bind, *, require_table: bool):
    ...
    if TABLE_NAME not in inspector.get_table_names():
        if require_table:
            raise RuntimeError(
                ...
                "refusing to stamp the evaluation run-token migration"
            )
        return None
    ...
    if get_primary_key is None:
        if any(
            ...
            raise RuntimeError(
                "Evaluation run-token primary key metadata is invalid"
            )
        primary_key = {
            "constrained_columns": [
                column["name"]
    ...
    if (
        ...
        raise RuntimeError("Evaluation run-token primary key metadata is invalid")
    return {
...
def _validate_existing_schema(snapshot) -> None:
    ...
    if column is not None:
        if column.get("computed") is not None:
            raise RuntimeError(
                "Refusing evaluation run-token migration: existing column "
    ...
    for constraint in snapshot["unique_constraints"]:
        ...
        if COLUMN_NAME in columns:
            ...
            raise RuntimeError(
                "Refusing evaluation run-token migration: unexpected unique "
    ...
    if index is not None:
        ...
        if actual_columns != (COLUMN_NAME,) or unique not in (False, 0):
            raise RuntimeError(
                "Refusing evaluation run-token migration: existing index "
...
def _validate_downgrade_indexes(snapshot) -> None:
    for name, index in snapshot["indexes"].items():
        ...
        display_name = name or "<unnamed>"
        raise RuntimeError(
            "Refusing evaluation run-token downgrade: unexpected index "
