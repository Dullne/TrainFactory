...
from ...deployment.deployment_service import deployment_service, get_default_xinference_endpoint
...
async def _start_deployment_in_background(deployment_id: str) -> None:
    ...
    try:
        ...
        await asyncio.to_thread(deployment_service.start_deployment, deployment_id)
...
def _build_deployment_config_with_external_api(
    ...
    if raw_config_api_config_id is not None and not isinstance(raw_config_api_config_id, str):
        raise HTTPException(
            status_code=400,
    ...
    if request_api_config_id and config_api_config_id and request_api_config_id != config_api_config_id:
        raise HTTPException(
            status_code=400,
            detail=(
...
class CreateDeploymentRequest(BaseModel):
    ...
    xinference_endpoint: Optional[str] = Field(default=None, description="Inference server endpoint (uses default if not specified)")
    deployment_name: Optional[str] = Field(default=None, description="Deployment name")
    gpu_id: Optional[int] = Field(default=None, ge=0, description="Preferred GPU device ID for Xinference shared mode")
    replica: int = Field(default=1, ge=1, le=8, description="Number of replicas (1-8)")
    gpu_memory_utilization: Optional[float] = Field(default=None, ge=0.05, le=1.0, description="GPU memory utilization (auto-calculated if not specified)")
    inference_framework: str = Field(default="xinference", pattern="^(vllm|sglang|xinference)$", description="Inference framework")
    enable_lora: bool = Field(default=False, description="Enable LoRA hot-loading")
    max_loras: int = Field(default=4, ge=1, le=16, description="Maximum number of LoRA adapters")
    max_lora_rank: int = Field(default=64, ge=8, le=256, description="Maximum LoRA rank")
...
class CreateContainerDeploymentRequest(BaseModel):
    ...
    model_id: str = Field(..., description="Model ID to deploy")
    deployment_name: Optional[str] = Field(default=None, description="Deployment name")
    gpu_id: Optional[int] = Field(default=None, ge=0, description="GPU device ID (auto-select if not specified)")
    port: Optional[int] = Field(default=None, ge=1024, le=65535, description="Port to expose (auto-assign if not specified)")
    replica: int = Field(default=1, ge=1, le=8, description="Number of replicas (1-8)")
    gpu_memory_utilization: Optional[float] = Field(default=None, ge=0.05, le=1.0, description="GPU memory utilization (auto-calculated if not specified)")
    inference_framework: str = Field(default="xinference", pattern="^(vllm|sglang|xinference)$", description="Inference framework")
    enable_lora: bool = Field(default=False, description="Enable LoRA hot-loading")
...
class RestartDeploymentRequest(BaseModel):
    ...
    reset_gpu: bool = Field(default=False, description="Reset GPU before restart (container mode only)")


class UpdateDeploymentConfigRequest(BaseModel):
    """Update deployment config request."""
    config: Optional[Dict[str, Any]] = Field(default=None, description="Deployment config (JSON)")
...
def _get_xinference_runtime_map(
    ...
    for endpoint, user_id in endpoints.items():
        try:
            ...
            for model in models:
                model_id = model.get("id") or model.get("model_uid") or model.get("model_id")
                if model_id:
                    model_map[model_id] = model
            if model_map:
                runtime_map_by_endpoint[endpoint] = model_map
        except Exception as e:
            logger.debug(f"Failed to fetch Xinference runtime info from {endpoint}: {e}")
...
def _deployment_to_response(
    ...
    unmanaged_binding = (
        config.get("runtime_managed") is False
    ...
    if container_name and deployment['status'] == 'running':
        ...
        if deploy_mode == 'shared' and deployment.get('inference_framework') == 'xinference' and model_uid and model_usage_map:
            ...
            if not usage:
                ...
                if not usage:
                    for key, value in container_usage.items():
                        if key.lower().endswith(model_uid.lower()) or (uid_match and key.lower().endswith(short_uid.lower())):
                            usage = value
                            break
            if usage:
        ...
        if not usage and deploy_mode != 'shared':
            ...
            if not usage:
                usage = docker_deployer.get_container_gpu_usage(container_name, gpu_id=effective_gpu_id)
    ...
    if runtime_map_by_endpoint and model_uid:
        ...
        if runtime_info:
            ...
            if isinstance(accelerators, list) and accelerators and effective_gpu_id is None:
    ...
    return DeploymentResponse(
        ...
        created_at=deployment['created_at'].isoformat() if deployment.get('created_at') else None,
        updated_at=deployment['updated_at'].isoformat() if deployment.get('updated_at') else None,
        started_at=deployment['started_at'].isoformat() if deployment.get('started_at') else None,
        stopped_at=deployment['stopped_at'].isoformat() if deployment.get('stopped_at') else None,
    )
...
async def create_deployment(
    ...
    try:
        ...
        resolved_external_api_config_id = (
            (request.external_api_config_id or "").strip()
            or (deployment_config or {}).get("external_api_config_id")
        )
...
async def list_deployments(
    ...
    sync: bool = Query(default=False, description="Sync status with actual service before returning"),
    current_user: Dict[str, Any] = Depends(get_current_user),
):
    ...
    if shared_xinference:
        ...
        for container_name in containers:
            model_usage_map[container_name] = docker_deployer.get_container_model_gpu_usage(container_name)

    runtime_map_by_endpoint = _get_xinference_runtime_map(shared_xinference) if shared_xinference else {}

    return DeploymentListResponse(
        deployments=[
            _deployment_to_response(d, gpu_usage, model_usage_map, runtime_map_by_endpoint)
...
class DiscoveredModel(BaseModel):
    ...
    model_type: Optional[str] = Field(default=None, description="Model type (embedding/rerank/llm)")
...
def discover_models(
    ...
    framework: str = Query(default="xinference", pattern="^(xinference|vllm|sglang)$", description="Inference framework"),
    current_user: Dict[str, Any] = Depends(get_current_user),
):
    """
        ...
        if framework == "xinference":
            ...
            try:
                ...
                if response.status_code == 200:
                    ...
                    model_name = model_path.split("/")[-1] if model_path else "sglang-model"
                    models.append(DiscoveredModel(
...
class BindExistingModelRequest(BaseModel):
    ...
    model_type: str = Field(default="embedding", description="Model type: embedding/rerank/llm")
    deployment_name: Optional[str] = Field(default=None, description="Deployment name")
    inference_framework: str = Field(default="xinference", pattern="^(vllm|sglang|xinference)$")
    container_name: Optional[str] = Field(default=None, description="Docker container name (for local deployments)")
    gpu_id: Optional[int] = Field(default=None, description="GPU device ID (for local deployments)")
    external_api_config_id: Optional[str] = Field(
        default=None,
        description="Optional external API config id used as tenant binding",
    )
...
async def get_deployment(
    ...
    return _deployment_to_response(deployment, runtime_map_by_endpoint=runtime_map_by_endpoint)
...
async def start_deployment(
    ...
    try:
        ...
        deployment = verify_resource_ownership(deployment, current_user, "Deployment")
        _validate_deployment_endpoint(deployment)
        deployment = await asyncio.to_thread(deployment_service.start_deployment, deployment_id)
        return _deployment_to_response(deployment)
    except SSRFError:
        raise
    except ValueError as e:
...
async def stop_deployment(
    ...
    try:
        ...
        deployment = verify_resource_ownership(deployment, current_user, "Deployment")
        _validate_deployment_endpoint(deployment)
        deployment = await asyncio.to_thread(deployment_service.stop_deployment, deployment_id)
        return _deployment_to_response(deployment)
    except ValueError as e:
        raise HTTPException(status_code=400, detail=str(e))
...
async def restart_deployment(
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Failed to restart deployment: {e}")
...
async def update_deployment_config(
    ...
    try:
        ...
        has_external_api_config_key = isinstance(request_config, dict) and "external_api_config_id" in request_config
        normalized_config = _build_deployment_config_with_external_api(
            request_config,
            None if has_external_api_config_key else deployment.get("external_api_config_id"),
            current_user,
        )
        deployment = deployment_service.update_deployment_config(deployment_id, normalized_config)
        return _deployment_to_response(deployment)
    except ValueError as e:
        raise HTTPException(status_code=400, detail=str(e))
    except Exception as e:
        raise HTTPException(status_code=500, detail=f"Failed to update deployment config: {e}")
...
async def sync_deployment_status(
    ...
    try:
        ...
        deployment = verify_resource_ownership(deployment, current_user, "Deployment")
        _validate_deployment_endpoint(deployment)
        deployment = deployment_service.sync_status(deployment_id)
        return _deployment_to_response(deployment)
    except ValueError as e:
        raise HTTPException(status_code=400, detail=str(e))
...
async def delete_deployment(
    ...
    try:
        ...
        deployment = verify_resource_ownership(deployment, current_user, "Deployment")
        _validate_deployment_endpoint(deployment)
        success = deployment_service.delete_deployment(deployment_id, force=force)
        if not success:
            raise HTTPException(status_code=500, detail=f"Failed to delete deployment: {deployment_id}")
...
async def create_config_for_deployment(
    ...
    try:
        ...
        deployment = verify_resource_ownership(deployment, current_user, "Deployment")
        _validate_deployment_endpoint(deployment)
        config = deployment_service.create_config_for_deployment(deployment_id)
        return {
