...
import { STATUS_INFO, STATUS_SUCCESS, STATUS_ERROR, TEXT_SECONDARY } from '@/theme'

const { Title } = Typography
const PAGE_SIZE = 50
const WORKSPACE_FILTERS = { tab: ['sync', 'api'] }
const PAGE_SIZES = [PAGE_SIZE]

export default function SyncConfigList() {
  const navigate = useNavigate()
  const { t } = useTranslation(['sync', 'common'])
  const workspace = useListWorkspace({
  ...
  const setPage = useCallback(
    (value: number) => setPagination(value, PAGE_SIZE, true),
    [setPagination]
  )
  ...
  const fetchConfigs = useCallback(
    async (requestedPage?: number, silent = false) => {
  ...
  const hasActiveConfigs = configs.some(
    (c) => c.is_active && ACTIVE_SYNC_STATUSES.includes(c.status)
  )
  ...
  const columns: ColumnsType<SyncConfig> = [
    ...
    },
    {
      ...
      render: (_, record) => {
        ...
        return (
          <div>
            <div style={{ fontSize: 12, lineHeight: '18px', whiteSpace: 'nowrap' }}>
              {formatText}
            </div>
            <Progress
              percent={percent}
              ...
              showInfo={false}
            />
            {subtitle && <span style={{ fontSize: 11, color: TEXT_SECONDARY }}>{subtitle}</span>}
          </div>
        )
      },
    },
    ...
    },
    {
      ...
      render: (name: string | null) =>
        name ? <Tag color="blue">{name}</Tag> : <span style={{ color: TEXT_SECONDARY }}>-</span>,
    },
    {
      ...
      render: (_, record) => (
        <Space size={4}>
          ...
          {record.is_active ? (
            ...
            <Popconfirm
              title={t('list.actions.confirmStart')}
              onConfirm={() => handleStart(record.task_id)}
            >
  ...
  return (
    <Tabs
      activeKey={workspace.values.tab || 'sync'}
      onChange={(tab) => workspace.update({ tab })}
      items={[
        {
          key: 'sync',
          label: (
            <span>
              <SyncOutlined /> {t('apiConfig.syncTab')}
            </span>
          ),
          children: (
            <div>
              <Row gutter={16} style={{ marginBottom: 20 }}>
                ...
                <Col span={6}>
                  <StatCard
                    title={`${t('list.stats.active')}${currentPageSuffix}`}
                    value={activeConfigs}
                ...
                <Col span={6}>
                  <StatCard
                    title={`${t('list.stats.syncing')}${currentPageSuffix}`}
                ...
                <Col span={6}>
                  <StatCard
                    title={`${t('list.stats.error')}${currentPageSuffix}`}
                    value={errorConfigs}
              ...
              <Table
                ...
                pagination={{
                  ...
                  onChange: (value) => setPagination(value, PAGE_SIZE),
                }}
              />
            </div>
          ),
