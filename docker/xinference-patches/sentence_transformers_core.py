...
class SentenceTransformerEmbeddingModel(EmbeddingModel, BatchMixin):
    ...
    def load(self):
        ...
        if (
            ...
            # Fix: Force Last Token Pooling for Qwen3-Embedding
            # sentence-transformers may default to mean pooling if config is not found
            if "embedding" in self.model_family.model_name.lower():
                for module in self._model:
                    if hasattr(module, 'pooling_mode_mean_tokens'):
                        logger.info("Forcing Last Token Pooling for Qwen3-Embedding")
